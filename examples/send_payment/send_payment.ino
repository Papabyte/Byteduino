...
void setup() {
 ...
 // setTestNet(); //uncomment this to switch to testnet
  ...
  WiFiMulti.addAP("My_SSID", "MyPassword");
...
void loop() {
  ...
  if (isDeviceConnectedToHub() && !payment_placed_in_buffer) {
    ...
    int error =  sendPayment(5000, "T3KHYLX2BGUQ2GV74YPMNU6LVVJD4VFB", 5564); // we send 5000 Bytes to T3KHYLX2BGUQ2GV74YPMNU6LVVJD4VFB and attribute 5564 as payment id
  
    switch (error) {
      case SUCCESS: // it doesn't mean that the payment is actually successful yet, the final status will be given by the onPaymentResult callback
